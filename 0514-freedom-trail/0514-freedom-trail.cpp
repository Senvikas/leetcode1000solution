...
int countSteps(int ringIndex,int i,int n)
{
    ...
    int wrapRoundDistance=n-distance;
    return min(distance,wrapRoundDistance);
}
    int solve(int ringIndex,int keyIndex,string& ring ,string& key)
    {
       ...
       return t[ringIndex][keyIndex];
        ...
        for(int i=0;i<ring.length();i++)
        {
            ...
            {
                int steps=1+countSteps(ringIndex,i,ring.length());
                int totalSteps=steps+solve(i,keyIndex+1,ring,key);
